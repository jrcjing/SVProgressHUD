...
+ (void)showImages:(NSArray *)images {
    [[self sharedView] showImages:images progress:SVProgressHUDUndefinedProgress status:nil];
}
...
- (void)showStatus:(NSString*)status {
    ...
    if(self.hudView.contentView.alpha != 1.0f){
        ...
        - (void)showImages:(NSArray *)images progress:(float)progress status:(NSString *)status{
            __weak SVProgressHUD *weakSelf = self;
            [[NSOperationQueue mainQueue] addOperationWithBlock:^{
                __strong SVProgressHUD *strongSelf = weakSelf;
                if(strongSelf){
